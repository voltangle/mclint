...
fn basic_code() -> Result<()> {
    ...
    assert_eq!(parser.parse().with_context(|| "Failed to parse data")?, vec![VariableDeclaration { name: "myVar".to_string(), default_val: MonkeyCExpression::Simple("Hm".to_string()), var_type: Some("String".to_string()), is_const: false },
